...
if [ "$1" = "run" ]; then
    ...
    echo "Preparando CalendLar configuração de produção.."

    createAppSettings

    echo "Starting CalendLar production server.."

    exec docker-php-entrypoint apache2-foreground

elif [ "$1" = "dev" ]; then

    echo "Preparando CalendLar DEV-MODE CONFIG.."


    createAppSettings
    sed -i "s/DEBUG_MODE    = FALSE/DEBUG_MODE    = TRUE/g" $PROJECT_DIR/config.php

    echo "Iniciando CalendLar Servidor de produção.."
